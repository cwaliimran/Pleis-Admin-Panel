...
import { RouteConfig } from '@/app/common/header/route-config';
...
const CompanyGuard = ({ children }: CompanyGuardProps) => {
  ...
  useEffect(() => {
    // Non-admin users don't need to select company/organization
    if (!isAdmin) {
      setState({
        isLoading: false,
        hasCompany: true,
    ...
    const checkSelections = () => {
      const company = CompanySelectionStorage.getSelectedCompany();
      const organization = CompanySelectionStorage.getSelectedOrganization();

      const hasValidCompany = Boolean(company?.value);
      const hasValidOrganization = Boolean(organization?.value);

      // Validate organization belongs to company
      const isOrganizationValid = !hasValidOrganization || !hasValidCompany || organization?.companyId === company?.value;

      setState({
        isLoading: false,
        hasCompany: hasValidCompany,
        hasOrganization: hasValidOrganization && isOrganizationValid,
        needsCompany: routeRequirements.requiresCompany,
        needsOrganization: routeRequirements.requiresOrganization,
      });
    };
    ...
    const unsubscribeCompany = CompanySelectionStorage.onCompanyChange(checkSelections);
    const unsubscribeOrganization = CompanySelectionStorage.onOrganizationChange(checkSelections);
  ...
  // Show company selection prompt
  if (state.needsCompany && !state.hasCompany && isAdmin) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <p className="text-muted-foreground mb-6">Please select a company from the dropdown in the header to continue.</p>
        </div>
      </div>
    );
  }
  ...
  if (state.needsOrganization && !state.hasOrganization && isAdmin) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="max-w-md text-center">
          <div className="bg-primary/10 mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full">
