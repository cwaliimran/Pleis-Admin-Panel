...
export default function OrgInfo({ organizationData }: any) {
  return (
    <>
      {/* First Image */}
      {organizationData?.otherInfo?.galleryMedia?.slice(0, 1).map((image: any, index: number) => (
      ...
      <div className="grid w-full grid-cols-12 gap-2">
        {organizationData?.otherInfo?.galleryMedia?.slice(1).map((image: any, index: number) => (
