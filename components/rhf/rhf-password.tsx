...
interface RHFPasswordFieldProps extends React.InputHTMLAttributes<HTMLInputElement> {
    name: string
    label?: string,
                            ...
                            {[0, 1, 2, 3].map((index) => {
                                ...
                                if (password) {
                                    if (strength.level > index) {
                                        if (strength.level >= 4) barColor = "bg-green-500"; // strong
                                        else if (strength.level >= 3) barColor = "bg-green-400"; // medium
...
export function getPasswordStrength(password: string) {
    ...
    if (score === 3 || score === 4) return { label: "Medium", color: "bg-green-400", level: 2 };
