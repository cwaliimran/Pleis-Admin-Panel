  ...
  useEffect(() => {
    if (watchedValue) {
      if (typeof watchedValue === 'string') {
        // If the value is a string (URL), use it directly as the preview
        setPreview(watchedValue);
      } else if (watchedValue?.[0] instanceof File) {
        // If the value is a FileList or array with a File, create an object URL
      ...
      <Controller
        ...
        render={({ field: { onChange, ref } }) => {
          ...
          return (
            <>
              {/* Upload Box */}
              <div className="relative rounded-md border border-dashed p-2">
                <div
                  className={`relative flex h-full w-full cursor-pointer items-center justify-center overflow-hidden border bg-gray-200 transition hover:opacity-80 dark:bg-gray-700 ${
                    hasError
