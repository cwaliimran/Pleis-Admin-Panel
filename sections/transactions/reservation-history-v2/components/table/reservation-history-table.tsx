...
import { countActiveFilters, FilterValues, ReservationFiltersPanel } from '../../advanced-filters';
import { FILTER_COUNT_BADGE } from '../../config/theme';
import { Reservation } from '../../types/types';
import { PresetId } from '../filters/filters';
import ReservationHistoryFilters from '../filters/reservation-history-filters';
import ReservationHistoryPagination from '../pagination/reservation-history-pagination';
import ReservationHistoryTableRow from './reservation-history-table-row';

const HEAD_LABEL = [
  { id: 'reservation', label: 'Reservation', align: 'left', sortable: true, sortKey: 'reservationId' },
  { id: 'venueSlot', label: 'Venue / Slot', align: 'left' },
  { id: 'guests', label: 'Guests', align: 'left' },
  { id: 'condition', label: 'Condition', align: 'left' },
  { id: 'voucher', label: 'Voucher', align: 'left' },
...
const ReservationHistoryTable: FC<ReservationHistoryTableProps> = ({
  data,
  loading,
  currentPage,
  totalPages,
  ...
  return (
    <Card className="dark:bg-secondary mt-5 mb-5 px-2 shadow-md md:px-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <h3 className="ml-2 text-xl font-semibold md:ml-0">Reservation List</h3>
      ...
      <div className="mt-4 min-h-[45vh] rounded-lg border">
        <Table className="w-full rounded-md border">
          ...
          <TableBodyWrapper loading={loading} colSpan={HEAD_LABEL.length} dataLength={sortedData?.length || 0}>
            {sortedData?.map((item) => (
              <ReservationHistoryTableRow key={item.id} item={item} onView={onView} />
            ))}
          </TableBodyWrapper>
        </Table>
      </div>
