...
const TriStateControl: FC<{ value: TriState; onChange: (value: TriState) => void; disabled?: boolean }> = ({ value, onChange, disabled }) => (
  <div className="inline-flex rounded-md border border-gray-300 p-0.5 dark:border-gray-600">
    {(['any', 'yes', 'no'] as TriState[]).map((option) => (
      <button
        key={option}
        type="button"
        disabled={disabled}
        onClick={() => onChange(option)}
        className={`cursor-pointer rounded px-3 py-1 text-xs font-medium capitalize transition disabled:cursor-not-allowed disabled:opacity-40 ${
...
const FilterFieldControl: FC<FilterFieldControlProps> = ({ field, options, value, onChange, disabled }) => {
  switch (field.type) {
    ...
    case 'tristate':
      return <TriStateControl value={value as TriState} onChange={onChange} disabled={disabled} />;
