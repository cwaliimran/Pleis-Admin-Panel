...
import { PaymentAttempt, TransactionDocument } from '../../types/types';
...
export const DocumentCard: FC<{ document: TransactionDocument; onView: () => void; onDownload: () => void }> = ({ document, onView, onDownload }) => (
...
const ATTEMPT_STATUS_LABEL: Record<PaymentAttempt['status'], string> = {
  succeeded: 'Succeeded',
  failed: 'Failed',
  refunded: 'Refunded',
};

const ATTEMPT_STATUS_CLASSNAME: Record<PaymentAttempt['status'], string> = {
...
export const PaymentAttemptRow: FC<{ attempt: PaymentAttempt }> = ({ attempt }) => (
  <div className="flex items-center gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700">
    ...
    <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${ATTEMPT_STATUS_CLASSNAME[attempt.status]}`}>
      {ATTEMPT_STATUS_LABEL[attempt.status]}
    </span>
    <p className="text-sm text-gray-700 dark:text-gray-300">{attempt.detail}</p>
  </div>
);
