...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, onEdit, onToggleStock, onToggleUpsell, isUpdating = false }) => {
    ...
    <>
      ...
      >
        ...
        <div className="relative flex h-44 shrink-0 items-center justify-center bg-linear-to-br from-[#2a599b] to-[#1300FF] text-6xl">
          {item?.imageUrl ? (
            <Image src={item?.imageUrl} alt={item?.name} height={400} width={400} className="h-full w-full object-cover" />
          ) : (
            categoryIcon
          )}
        ...
        <div className="flex grow flex-col p-5">
          ...
          <div className="mb-0 grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 dark:border-gray-700 dark:bg-gray-800/50">
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Out of Stock</span>
              <button
                title="Out of stock"
                type="button"
                ...
                disabled={isUpdating}
                className={cn(
                  'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none',
              ...
              >
                <span
                  className={cn(
                    ...
                    !item.isInStock ? 'translate-x-4' : 'translate-x-0'
                  )}
                />
              </button>
            </div>
            ...
            {/* Upsell Item Toggle */}
            <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 dark:border-gray-700 dark:bg-gray-800/50">
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">Upsell Item</span>
              <button
                title="Upsell Item"
                type="button"
                onClick={() => {
                  if (!item.isInStock) {
                    showError('Cannot enable upsell for out of stock items');
                    return;
                  }
                  onToggleUpsell(item);
                }}
                ...
                className={cn(
                  'relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none',
                  item.isUpsell ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600',
                  isUpdating && 'cursor-not-allowed opacity-50'
                )}
              >
                <span
                  className={cn(
                    'pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                    item.isUpsell ? 'translate-x-4' : 'translate-x-0'
                  )}
                />
              </button>
            </div>
