...
export const transformApiOrderToFrontend = (apiOrder: ApiOrder): Order => {
  ...
  let location = '';
  if (apiOrder.pickupType === 'tableService') {
    location = apiOrder.tableNumber ? `Table ${apiOrder.tableNumber}` : 'Table Service';
  ...
  return {
    ...
    deliveryType,
    location,
    userName: `${apiOrder.user.firstName} ${apiOrder.user.lastName}`.trim(),
