import { Order } from './types';

// New Orders - Orders that have just been placed and need to be accepted (status: 'sent')
export const MOCK_NEW_ORDERS: Order[] = [
  {
    id: '1',
    deliveryType: 'table',
    location: 'Table 12',
    userName: 'Sarah Johnson',
    userHandle: '@sarahj',
    userEmail: 'sarah.johnson@email.com',
    status: 'sent',
    totalCost: 34.1,
    isVIP: true,
    tierName: 'Gold Member',
    partySize: 2,
    notes: 'No croutons on one salad. Extra dressing on the side.',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'credit-card',
      status: 'completed',
      transactionId: 'TXN-001-2024-ABC123',
      cardLast4: '4242',
      cardBrand: 'visa',
      paidAt: 'Dec 15, 2024, 11:30 AM',
      processingFee: 1.28,
      tip: 8.5,
    },
    items: [
      { id: '1-1', name: 'Caesar Salad', quantity: 2, price: 24.0 },
      { id: '1-2', name: 'Grilled Chicken', quantity: 1, price: 10.1 },
    ],
  },
  {
    id: '2',
    deliveryType: 'togo',
    location: 'To Go',
    userName: 'Michael Chen',
    userHandle: '@mchen',
    userEmail: 'michael.chen@email.com',
    status: 'sent',
    totalCost: 28.0,
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'apple-pay',
      status: 'completed',
      transactionId: 'TXN-002-2024-DEF456',
      paidAt: 'Dec 15, 2024, 12:15 PM',
      processingFee: 0.84,
    },
    items: [
      { id: '2-1', name: 'Margherita Pizza', quantity: 1, price: 16.0 },
      { id: '2-2', name: 'Tiramisu', quantity: 1, price: 12.0 },
    ],
  },
  {
    id: '3',
    deliveryType: 'table',
    location: 'Table 5',
    userName: 'Emma Williams',
    userHandle: '@emmaw',
    userEmail: 'emma.williams@email.com',
    status: 'sent',
    totalCost: 87.25,
    partySize: 4,
    paymentType: 'pay-later',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'cash',
      status: 'pending',
    },
    items: [
      { id: '3-1', name: 'Ribeye Steak', quantity: 2, price: 64.0 },
      { id: '3-2', name: 'House Wine', quantity: 2, price: 18.0 },
      { id: '3-3', name: 'Chocolate Cake', quantity: 1, price: 5.25 },
    ],
  },
  {
    id: '4',
    deliveryType: 'table',
    location: 'Table 8',
    userName: 'James Anderson',
    userHandle: '@janderson',
    userEmail: 'james.anderson@email.com',
    status: 'sent',
    totalCost: 56.75,
    partySize: 6,
    isPreorder: true,
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'debit-card',
      status: 'completed',
      transactionId: 'TXN-004-2024-GHI789',
      cardLast4: '1234',
      cardBrand: 'mastercard',
      paidAt: 'Dec 15, 2024, 10:00 AM',
      processingFee: 1.7,
      tip: 10.0,
    },
    items: [
      { id: '4-1', name: 'Fish & Chips', quantity: 2, price: 32.0 },
      { id: '4-2', name: 'Cola', quantity: 2, price: 8.0 },
      { id: '4-3', name: 'Ice Cream', quantity: 2, price: 16.75 },
    ],
  },
  {
    id: '5',
    deliveryType: 'togo',
    location: 'To Go',
    userName: 'Olivia Martinez',
    userHandle: '@oliviam',
    userEmail: 'olivia.martinez@email.com',
    status: 'sent',
    totalCost: 15.5,
    isPreorder: true,
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'google-pay',
      status: 'completed',
      transactionId: 'TXN-005-2024-JKL012',
      paidAt: 'Dec 15, 2024, 12:45 PM',
      processingFee: 0.47,
    },
    items: [{ id: '5-1', name: 'Club Sandwich', quantity: 1, price: 15.5 }],
  },
];

// In Progress Orders - Orders that are being prepared (status: 'preparing')
export const MOCK_IN_PROGRESS_ORDERS: Order[] = [
  {
    id: '6',
    deliveryType: 'table',
    location: 'Table 3',
    userName: 'David Brown',
    userHandle: '@dbrown',
    userEmail: 'david.brown@email.com',
    status: 'preparing',
    totalCost: 34.0,
    partySize: 2,
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'paypal',
      status: 'completed',
      transactionId: 'TXN-006-2024-MNO345',
      paidAt: 'Dec 15, 2024, 1:00 PM',
      processingFee: 1.02,
      tip: 5.0,
    },
    items: [
      { id: '6-1', name: 'Spaghetti Carbonara', quantity: 1, price: 22.0 },
      { id: '6-2', name: 'Garlic Bread', quantity: 1, price: 12.0 },
    ],
  },
  {
    id: '7',
    deliveryType: 'togo',
    location: 'To Go',
    userName: 'Sophia Taylor',
    userHandle: '@sophiat',
    userEmail: 'sophia.taylor@email.com',
    status: 'preparing',
    totalCost: 45.25,
    isVIP: true,
    tierName: 'Platinum Member',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'credit-card',
      status: 'completed',
      transactionId: 'TXN-007-2024-PQR678',
      cardLast4: '8888',
      cardBrand: 'amex',
      paidAt: 'Dec 15, 2024, 1:15 PM',
      processingFee: 1.36,
      tip: 9.0,
    },
    items: [
      { id: '7-1', name: 'Sushi Roll', quantity: 2, price: 28.0 },
      { id: '7-2', name: 'Miso Soup', quantity: 1, price: 8.0 },
      { id: '7-3', name: 'Green Tea', quantity: 1, price: 9.25 },
    ],
  },
  {
    id: '8',
    deliveryType: 'table',
    location: 'Table 15',
    userName: 'Liam Wilson',
    userHandle: '@liamw',
    userEmail: 'liam.wilson@email.com',
    status: 'preparing',
    totalCost: 68.5,
    partySize: 3,
    notes: 'Extra BBQ sauce. No onions in coleslaw.',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'credit-card',
      status: 'processing',
      cardLast4: '5555',
      cardBrand: 'discover',
    },
    items: [
      { id: '8-1', name: 'BBQ Ribs', quantity: 2, price: 46.0 },
      { id: '8-2', name: 'Coleslaw', quantity: 2, price: 12.0 },
      { id: '8-3', name: 'Lemonade', quantity: 2, price: 10.5 },
    ],
  },
  {
    id: '9',
    deliveryType: 'table',
    location: 'Table 10',
    userName: 'Isabella Garcia',
    userHandle: '@isabellag',
    userEmail: 'isabella.garcia@email.com',
    status: 'waiting-payment',
    totalCost: 38.0,
    partySize: 2,
    isVIP: true,
    tierName: 'Gold Member',
    paymentType: 'pay-later',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'cash',
      status: 'pending',
    },
    items: [
      { id: '9-1', name: 'Seafood Paella', quantity: 1, price: 28.0 },
      { id: '9-2', name: 'Sangria', quantity: 1, price: 10.0 },
    ],
  },
];

// Completed Orders - Orders that are paid or canceled
export const MOCK_COMPLETED_ORDERS: Order[] = [
  {
    id: 'past1',
    deliveryType: 'table',
    location: 'Table 7',
    userName: 'Ethan Moore',
    userHandle: '@ethanm',
    userEmail: 'ethan.moore@email.com',
    status: 'paid',
    totalCost: 31.5,
    partySize: 1,
    completedAt: 'Today, 2:45 PM',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'credit-card',
      status: 'completed',
      transactionId: 'TXN-PST1-2024-BCD890',
      cardLast4: '1111',
      cardBrand: 'visa',
      paidAt: 'Today, 2:45 PM',
      processingFee: 0.95,
      tip: 6.3,
    },
    items: [
      { id: 'past1-1', name: 'Burger & Fries', quantity: 1, price: 18.5 },
      { id: 'past1-2', name: 'Milkshake', quantity: 1, price: 13.0 },
    ],
  },
  {
    id: 'past2',
    deliveryType: 'togo',
    location: 'To Go',
    userName: 'Mia Thomas',
    userHandle: '@miat',
    userEmail: 'mia.thomas@email.com',
    status: 'canceled',
    totalCost: 12.0,
    completedAt: 'Today, 1:30 PM',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'google-pay',
      status: 'refunded',
      transactionId: 'TXN-PST2-2024-EFG123',
      paidAt: 'Today, 1:00 PM',
      refundedAt: 'Today, 1:30 PM',
      refundAmount: 12.0,
      processingFee: 0.36,
    },
    items: [{ id: 'past2-1', name: 'Greek Salad', quantity: 1, price: 12.0 }],
  },
  {
    id: 'past3',
    deliveryType: 'table',
    location: 'Table 20',
    userName: 'Noah Robinson',
    userHandle: '@noahr',
    userEmail: 'noah.robinson@email.com',
    status: 'paid',
    totalCost: 52.75,
    partySize: 2,
    completedAt: 'Today, 12:00 PM',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'apple-pay',
      status: 'completed',
      transactionId: 'TXN-PST3-2024-VWX234',
      paidAt: 'Today, 12:00 PM',
      processingFee: 1.58,
    },
    items: [
      { id: 'past3-1', name: 'Lobster Tail', quantity: 2, price: 42.0 },
      { id: 'past3-2', name: 'Champagne', quantity: 1, price: 10.75 },
    ],
  },
  {
    id: 'past4',
    deliveryType: 'table',
    location: 'Table 25',
    userName: 'Ava Lee',
    userHandle: '@avalee',
    userEmail: 'ava.lee@email.com',
    status: 'paid',
    totalCost: 64.0,
    partySize: 4,
    isVIP: true,
    tierName: 'Platinum Member',
    completedAt: 'Today, 11:30 AM',
    createdAt: new Date().toISOString(),
    paymentInfo: {
      method: 'debit-card',
      status: 'completed',
      transactionId: 'TXN-PST4-2024-YZA567',
      cardLast4: '7777',
      cardBrand: 'mastercard',
      paidAt: 'Today, 11:30 AM',
      processingFee: 1.92,
      tip: 12.8,
    },
    items: [
      { id: 'past4-1', name: 'Filet Mignon', quantity: 2, price: 52.0 },
      { id: 'past4-2', name: 'Red Wine', quantity: 2, price: 12.0 },
    ],
  },
];
