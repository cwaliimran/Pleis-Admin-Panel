...
export default function ReservationHeader({ date, onDateChange, range, onRangeChange }: ReservationHeaderProps) {
  const defaultValues: FormValues = { date: date, range: range };
  ...
  const methods = useForm<FormValues>({
    ...
    values: { date, range }, // Control form state from outside
  });

  const { handleSubmit, watch, setValue } = methods;

  // Watch for changes in date and range fields
  const watchedDate = watch('date');
  const watchedRange = watch('range');

  // Sync internal form state changes with external state handlers
  React.useEffect(() => {
    if (watchedDate !== date) {
      onDateChange(watchedDate);
    }
  }, [watchedDate, date, onDateChange]);

  React.useEffect(() => {
    if (watchedRange !== range) {
      onRangeChange(watchedRange);
    }
  }, [watchedRange, range, onRangeChange]);
