...
const AddOtherDetailsModal: React.FC<AddOtherDetailsModalProps> = ({
  ...
  const initialGalleryMediaInfo =
    newOrganization?.otherInfo?.galleryMediaInfo || [];
    ...
    defaultValues: newOrganization?.otherInfo
      ? {
          ...
          categories:
            ...
            [],
          existingGallery:
            initialGalleryMediaInfo.map((m: any) => m.name) || [],
  ...
  const watchGalleryImages =
    useWatch({ control: methods.control, name: 'galleryImages' }) || [];
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    try {
      ...
      const initialExistingNames =
        initialGalleryMediaInfo.map((m: any) => m.name) || [];
      removed = initialExistingNames.filter(
        (n: string) => !formData.existingGallery.includes(n)
      );
...
const GalleryModal: React.FC<GalleryModalProps> = ({
  ...
  useEffect(() => {
    ...
    const existingImages: GalleryItem[] = initialExisting.map((e) => ({
      url: e.url,
      key: e.name,
      name: e.name,
    }));
  ...
  const handleAdd = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((f) => {
      if (!allowedTypes.includes(f.type)) {
        showError(
          `Invalid file type for ${f.name}. Only JPEG, PNG, GIF, or WEBP allowed.`
        );
        return;
      }
