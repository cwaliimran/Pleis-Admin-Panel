...
const PageViewTrendsChart = ({ data, isLoading = false }: { data: TimelinePoint[]; isLoading?: boolean }) => {
  const chartData = data.length > 0 ? data : fallbackTimeline();

  return (
    <Card className="h-[430px] dark:bg-[#171717]">
      <CardHeader>
        <h3 className="text-xl font-semibold">Page View Trends</h3>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 20 }}>
                ...
                <Bar dataKey="value" name="Page Views" fill="#2563EB" radius={[6, 6, 0, 0]} barSize={34} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
...
export default PageViewTrendsChart;
