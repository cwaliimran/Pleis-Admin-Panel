...
import { useGetOrgNotificationsByIdQuery } from '@/store/Reducer/organization';
import Image from 'next/image';
import { useState } from 'react';
import NotificationCardSkelton from '../event/components/notification-card-skelton';

const LIMIT = 30;

const UserNotifications = ({ organizationId }: { organizationId: string }) => {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useGetOrgNotificationsByIdQuery({ id: organizationId, page: page - 1, limit: LIMIT });
  ...
  return (
    <div>
      <h2 className="mb-4 text-xl font-bold">Organization Notifications</h2>
            ...
            <Card key={notif?._id} className="border border-gray-200 shadow transition-shadow hover:shadow-md dark:border-gray-800 dark:bg-[#18181c]">
              ...
              <CardContent>
                ...
                <div className="mb-2 flex flex-wrap gap-2 text-xs">
                  ...
                  {notif.ageRange && notif.ageRange.length > 0 && (
                    <span className="rounded bg-gray-100 px-2 py-0.5 font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-200">
