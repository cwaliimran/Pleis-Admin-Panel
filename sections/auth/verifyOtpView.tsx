...
function VerifyOtpView() {
  ...
  const [forgotPasswordData, { isLoading: isLoadingForgotPassword }] = useSendOtpMutation();
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      router.push(`/user/reset-password?email=${encodeURIComponent(email)}${resetToken ? `&rst=${encodeURIComponent(resetToken)}` : ''}`);
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      console.log('Failed to add category:', errorMessage);
      showError(errorMessage);
      ...
      <div className="border-border flex w-full max-w-5xl overflow-hidden rounded-xl border bg-white/30 shadow-2xl backdrop-blur-md transition-all dark:bg-black/30">
        ...
        >
          <div className="space-y-4 text-center">
            <h1 className="text-4xl font-extrabold tracking-tight">Verify Otp</h1>
            <p className="mx-auto max-w-sm text-lg text-gray-300">We&#39;ll send you a verification code via email.</p>
          </div>
        </motion.div>
        ...
        >
          <h2 className="mb-1 text-center text-3xl font-extrabold">Verify Your Otp</h2>
          <p className="text-muted-foreground mb-6 text-center text-sm">Enter the verification code sent to your email.</p>

          <FormProvider methods={methods} onSubmit={onSubmit}>
            <div className="space-y-4">
              <RHFTextField name="otp" type="number" placeholder="Enter OTP" className="h-[45px] rounded-md" />
              ...
              <p className="text-muted-foreground text-center text-sm font-medium">
                Didn&#39;t receive code?{' '}
                <span
                  onClick={isLoadingForgotPassword ? undefined : handleResendOtp}
