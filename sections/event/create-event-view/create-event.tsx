...
const CreateEventView = (props: any) => {
  ...
  const { data: event = {}, isLoading: eventLoading = false } =
    useGeteventByIdQuery(id ?? skipToken);

  const { data: { data: organizations = [] } = {} } = useGetOrganizationQuery({
    page: 0,
    limit: 100,
  });

  const { data: { data: venues = [] } = {}, refetch: refetchVenues } =
  ...
  const { data: { data: tagsd = [] } = {} } = useGetTagsQuery({
    page: 0,
    limit: 100,
  });
