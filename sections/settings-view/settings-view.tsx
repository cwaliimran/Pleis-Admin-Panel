...
const SettingsView = () => {
  ...
  console.log('🔄 SettingsView rendered. States:', {
    selectedCompanyId,
    selectedCompanyName: selectedCompany?.companyDetails?.name,
    clubTitle,
    selectedModel,
    pointValue,
  });
  ...
  // Listen for company changes from header and update selected company
  useEffect(() => {
    const handleCompanyChange = () => {
      const stored = localStorage.getItem('selectedCompany');
      console.log('📦 localStorage selectedCompany:', stored);
      try {
        const company = stored ? JSON.parse(stored) : null;
        console.log('🏢 Parsed company object:', company);
        console.log('🆔 Setting company ID:', company?.value);
  ...
  useEffect(() => {
    console.log('🔍 Looking for company. List:', companyList?.length, 'items, ID:', selectedCompanyId);
  ...
  useEffect(() => {
    console.log('🎨 Prefilling fields for company:', selectedCompany);

    if (!selectedCompany) {
      // Reset fields when no company is selected
      setClubTitle('');
      setSelectedModel('');
      setPointValue(0);
      return;
    ...
    console.log('⚙️ Loyalty settings:', loyaltySettings);

    if (loyaltySettings) {
      console.log('✏️ Setting values:', {
        title: loyaltySettings.title,
        model: loyaltySettings.model,
        pointValue: loyaltySettings.pointValuePercentage,
      });
      ...
      // Reset to defaults if no loyalty settings exist
      console.log('❌ No loyalty settings, resetting to defaults');
      setClubTitle('');
      setSelectedModel('');
      setPointValue(0);
    }
