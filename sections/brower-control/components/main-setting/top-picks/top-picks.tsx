...
const TopPicks = ({ heading, viewAll, fixLength }: PromoManagerProps) => {
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const activeIndex = promoEvents.findIndex((promo: any) => promo._id === active.id);
      const overIndex = promoEvents.findIndex((promo: any) => promo._id === over.id);
    ...
    >
      <div className="p-0">
        <div className="mx-auto max-w-full">
            ...
            <div className="space-y-2">
              {isLoading ? (
                ...
                (fixLength ? promoEvents.slice(0, 10) : promoEvents).map((promo: any) => (
                  <DraggablePromoItem key={promo?._id} promo={promo} onEdit={openEditModal} onDelete={handleDelete} />
                ))
              )}
            </div>
          </SortableContext>
          ...
          {/* Modals */}
          {openModal.value && (
            <PromoSectionModal open={openModal.value} isEdit={editModal.value} onClose={handleCloseModal} selectedData={editingPromo} />
          )}
