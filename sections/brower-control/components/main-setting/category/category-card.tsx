...
export function CategoryCard({ category, onEdit, onDelete }: CategoryCardProps) {
  const getStatusText = () => {
    const parts = []
    parts.push(`${category.itemCount} ${category.itemCount === 1 ? "item" : "items"}`)
  ...
  return (
    <div
      className={`${categoryColors[category.type]} border-l-4 bg-white border border-gray-200 rounded-lg p-4 transition-all hover:shadow-md`}
    >
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-2 mt-2">
              <Badge variant="secondary" className="text-xs">
                {category.type.charAt(0).toUpperCase() + category.type.slice(1)}: {category.itemCount}
              </Badge>
            </div>
          </div>
        </div>
