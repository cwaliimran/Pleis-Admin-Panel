...
import { useUpdatePinnedContentMutation } from '@/store/Reducer/pinned-content-api';
...
export function CategoryCard({
  ...
  const [updatePinnedContent, { isLoading: isToggling }] = useUpdatePinnedContentMutation();

  const handleStatusToggle = async () => {
    const newStatus = category?.status === 'active' ? 'inactive' : 'active';
    try {
      const response = await updatePinnedContent({
        id: category?._id,
        status: newStatus,
      }).unwrap();
      ...
      <div className="dark:bg-secondary flex scale-105 rotate-1 items-center justify-between rounded-lg border border-l-4 border-gray-200 border-l-blue-500 bg-white p-4 opacity-95 shadow-lg dark:border-gray-800">
        <div className="flex flex-1 items-center space-x-4">
          <div className="flex-1">
            <h3 className="text-md font-semibold text-gray-900 capitalize dark:text-white">{category?.object?.title || ''}</h3>
            <p className="mt-0 text-sm text-gray-600 dark:text-white">{category?.type || 'N/A'}</p>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {/* Right: toggle + contentType + drag + actions */}
        <div className="flex items-center space-x-3">
          {/* ✅ Status toggle + contentType label */}
          {category?.contentType && (
            <div className="flex items-center space-x-2">
              {/* Same pattern as TwoFactorAuth */}
              <div className="relative flex items-center gap-2">
                <p className="text-sm text-gray-600 dark:text-white">{category?.contentType}</p>
                  ...
                  <>
                    ...
                    <div
                      ...
                      className={`peer relative h-6 w-11 cursor-pointer rounded-full after:absolute after:top-0.5 after:left-0.5 after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] ${
                        category?.status === 'active' ? 'bg-primary after:translate-x-full after:border-white' : 'bg-gray-200'
                      }`}
                    />
                  </>
                )}
