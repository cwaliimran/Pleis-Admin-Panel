...
const HEAD_LABEL = [
  ...
  { id: 'discountValue', label: 'Discount Value', align: 'left' },
  { id: 'maxDiscountCap', label: 'Max Discount Cap', align: 'left' },
  { id: 'maxUsage', label: 'Max Usage', align: 'left' },
  { id: 'usedCount', label: 'Used Count', align: 'left' },
...
const PromoCodeTable: FC<SamplePageProps> = ({
  data = [],
  meta,
  loading,
  handleDelete,
      ...
      <div className="grid grid-cols-12">
        <Card className="dark:bg-secondary col-span-12 mt-5 mb-5 px-2 shadow-md md:px-8 lg:col-span-12">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <h3 className="ml-2 text-xl font-semibold md:ml-0">Promo Code List</h3>
            ...
            <Table className="w-full rounded-md border">
              ...
              <TableBodyWrapper loading={loading} colSpan={HEAD_LABEL.length} dataLength={sortedData?.length || 0}>
                {sortedData?.map((item, idx) => (
                  <PromoCodeTableRow key={item?._id || idx} item={item} handleDelete={handleDelete} handleEdit={handleEdit} />
                ))}
              </TableBodyWrapper>
            </Table>
          </div>
