...
const ManagerProfileSection = () => {
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    try {
      ...
      const basicFields: (keyof ProfileFormData)[] = ['firstName', 'lastName'];
    ...
    <div className="mt-5 min-h-[87vh] md:mt-0 md:p-6">
      <div className="max-w-3xl md:mx-auto">
        <Card className="dark:bg-secondary border-gray-200 bg-white shadow-sm dark:border-none">
          <CardHeader>
            <CardTitle className="text-2xl font-semibold text-gray-900 dark:text-white">Manager Profile</CardTitle>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">Manage your personal information.</p>
          </CardHeader>

          <CardContent className="space-y-6 pt-0 pb-0 md:px-8">
            <FormProvider methods={methods} onSubmit={onSubmit}>
              {/* Hidden file input */}
              <Input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileSelect} className="hidden" />

              {/* Avatar Section */}
              <div className="flex items-center space-x-4 pb-6 md:space-x-6">
                <Avatar className="h-20 w-20">
                  <AvatarImage src={avatarUrl ?? undefined} />
                  <AvatarFallback className="bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-white">
                    <span className="text-lg font-semibold">
                      {user?.basicInfo?.firstName?.charAt(0) ? user?.basicInfo?.firstName?.charAt(0).toUpperCase() : 'M'}
                    </span>
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-gray-900 dark:text-white">Profile Picture</h3>
                  <p className="mt-1 text-xs text-gray-600 dark:text-gray-400">Click to upload a new profile picture</p>
                  <Button type="button" onClick={handleAvatarChange} variant="outline" size="sm" className="mt-3" disabled={imageUploading}>
                  ...
                  render={({ field, fieldState }) => (
                    <div>
                      ...
                      <PhoneInput
                        ...
                        buttonClass="!bg-transparent !border-none !shadow-none px-2 text-gray-800 dark:text-white"
              ...
              <div className="flex items-center justify-end gap-3 pt-6">
                {updateUserLoading || imageUploading ? (
                  <Button type="button" className="bg-primary hover:bg-primary h-10 cursor-not-allowed px-6 text-white">
                  ...
                  <Button
                    ...
                    className="bg-primary hover:bg-primary h-10 px-6 text-white"
                  >
                    Save Changes
                  </Button>
                )}
...
export default ManagerProfileSection;
