...
export const OrganizerSubscriptionView: React.FC = () => {
  ...
  // ============================================================================
  useEffect(() => {
    if (!userSubscriptionData || isPrefilled || isFreePlan) return;
    ...
    setIsPrefilled(true);
  }, [userSubscriptionData, isPrefilled, isFreePlan]);
  ...
  }, [selectedModules, includeAnalytics, organizationCount, customOrgCount, isCustomOrgSelected, billingCycle, dynamicPricing, isFreePlan]);
  ...
  const isButtonDisabled = selectedModules.length === 0 && !includeAnalytics && !hasActiveSubscription && !isFreePlan;
  ...
  return (
    <section className="min-h-screen">
      ...
      <div className="space-y-6 rounded-b-2xl px-0 py-8">
        ...
        {/* Current Subscription & Prorated Upgrade - Only for Paid Users */}
        {!isFreePlan && hasActiveSubscription && subscriptionAnalysis?.currentSubscription && userSubscriptionData && (
          <>
            {!hasUserMadeChanges ? (
        ...
        {/* Next Recurring Display - Only for Paid Users */}
        {!isFreePlan && subscriptionAnalysis?.nextRecurring && hasActiveSubscription && (
          <NextRecurringBox nextRecurring={subscriptionAnalysis.nextRecurring} isFreePlan={isDowngradingToFree} />
        )}
...
//   // ============================================================================
//   // PRICE BREAKDOWN CALCULATION (FOR FREE USERS OR NEW SELECTIONS)
//   // ============================================================================
//   const priceBreakdown = useMemo(() => {
//     if (!dynamicPricing || selectedModules.length === 0) return null;
...
//     if (subscriptionAnalysis?.isFreePlan || (selectedModules.length === 0 && !includeAnalytics && hasActiveSubscription)) {
//       setShowConfirmModal(true);
//       return;
//     }
...
//         return 'Switch to Free Plan';
//       }
//       return isFreePlan ? 'Continue with Free Plan' : 'Select modules to continue';
//     }
...
//   const isDowngradingToFree = useMemo(() => {
//     return hasActiveSubscription && selectedModules.length === 0 && !includeAnalytics;
//   }, [hasActiveSubscription, selectedModules, includeAnalytics]);
...
//         {/* Downgrade to Free Warning Banner */}
//         {isDowngradingToFree && (
//           <InfoBanner
//             variant="warning"
//             icon="⚠️"
...
//         {/* Price Breakdown - Show for Free Users or When Modules Selected */}
//         {priceBreakdown && (isFreePlan || selectedModules.length > 0) && <PriceBreakdownBox breakdown={priceBreakdown} />}
...
// //         {/* Current Subscription & Prorated Upgrade - Conditional Layout */}
// //         {hasActiveSubscription && subscriptionAnalysis?.currentSubscription && userSubscriptionData && (
// //           <>
// //             {!hasUserMadeChanges ? (
// //               // Show only current subscription in full width when no changes
// //               <CurrentSubscriptionBox
// //                 subscription={subscriptionAnalysis.currentSubscription}
// //                 startDate={userSubscriptionData.startDate}
// //                 endDate={userSubscriptionData.endDate}
...
// //               />
// //             ) : (
// //               // Show both side-by-side when user makes changes
// //               <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
