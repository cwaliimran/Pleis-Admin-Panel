    ...
    <div className="rounded-xl border-2 border-green-200 bg-green-50 p-6 shadow-sm dark:border-green-800 dark:bg-green-950/30">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-green-900 dark:text-green-100">Upgrade Cost</h3>
        </div>
      ...
      {/* What's Being Added */}
      <div className="mb-4">
        <p className="mb-3 text-sm font-semibold text-green-800 dark:text-green-300">You&apos;re adding:</p>
        <div className="space-y-2">
          {/* Added Modules */}
          {hasModules && (
            <div className="flex items-start gap-2 rounded-md bg-white p-3 dark:bg-green-900/20">
              <Plus className="mt-0.5 h-4 w-4 text-green-600 dark:text-green-500" />
          ...
          {hasAnalytics && (
            <div className="flex items-start gap-2 rounded-md bg-white p-3 dark:bg-green-900/20">
              <Plus className="mt-0.5 h-4 w-4 text-green-600 dark:text-green-500" />
              <div className="flex-1">
                <span className="text-sm font-medium text-green-800 dark:text-green-300">Advanced Analytics</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mb-4 space-y-2 rounded-lg border border-green-300 bg-white p-4 dark:border-green-700 dark:bg-green-900/30">
        <p className="mb-3 text-xs font-semibold tracking-wide text-green-700 uppercase dark:text-green-400">Cost Breakdown</p>

        {/* Module Cost */}
        {upgrade.moduleCost > 0 && (
          <div className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="text-green-700 dark:text-green-400">Added Module{upgrade.modulesAdded.length > 1 ? 's' : ''}:</span>
              <span className="font-medium text-green-900 dark:text-green-100">€{upgrade.moduleCost.toFixed(2)}</span>
            </div>
            <div className="ml-4 text-xs text-green-600 dark:text-green-500">(No bundle discount on upgrades)</div>
          </div>
        )}
        ...
        {upgrade.orgCost > 0 && (
          <div className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="text-green-700 dark:text-green-400">Added Organization{upgrade.orgsAdded > 1 ? 's' : ''}:</span>
              <span className="font-medium text-green-900 dark:text-green-100">€{upgrade.orgCost.toFixed(2)}</span>
            </div>
            <div className="ml-4 text-xs text-green-600 dark:text-green-500">(With {upgrade.multiOrgDiscountPercent}% rate applied)</div>
          </div>
        )}
        ...
        {upgrade.analyticsCost > 0 && (
          <div className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="text-green-700 dark:text-green-400">Analytics:</span>
              <span className="font-medium text-green-900 dark:text-green-100">€{upgrade.analyticsCost.toFixed(2)}</span>
            </div>
            <div className="ml-4 text-xs text-green-600 dark:text-green-500">(No discounts apply)</div>
          </div>
        )}

        {/* Yearly Discount Applied */}
        {upgrade.yearlyDiscountPercent > 0 && (
          <div className="mt-2 rounded-md bg-green-100 p-2 text-xs dark:bg-green-900/40">
