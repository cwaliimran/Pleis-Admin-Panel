...
const SubscriptionModal = ({ open, onClose, selectedData, pricingData }: SubscriptionModalProps) => {
  ...
  const numberOfOrganizations = useMemo(() => {
    ...
    console.log('🔢 Number of Organizations watched (converted to number):', num, 'Type:', typeof num);
    return num;
  ...
  const pricingCalculation = useMemo(() => {
    console.log('🔄 Recalculating pricing with numberOfOrganizations:', numberOfOrganizations, 'Type:', typeof numberOfOrganizations);
    ...
    console.log('💰 Multi-Org Pricing Config:', freshMultiOrgPricing);

    const numOrgs = Number(numberOfOrganizations);
    console.log('🎯 Checking conditions with numOrgs:', numOrgs, 'Type:', typeof numOrgs);

    if (numOrgs === 1) {
      orgPricingPercent = freshMultiOrgPricing.oneOrg;
      console.log('✅ Match: 1 org → ' + orgPricingPercent + '%');
    } else if (numOrgs === 2) {
      orgPricingPercent = freshMultiOrgPricing.twoOrgs;
      console.log('✅ Match: 2 orgs → ' + orgPricingPercent + '%');
    } else if (numOrgs === 3) {
      orgPricingPercent = freshMultiOrgPricing.threeOrgs;
      console.log('✅ Match: 3 orgs → ' + orgPricingPercent + '%');
    } else if (numOrgs === 4) {
      orgPricingPercent = freshMultiOrgPricing.fourOrgs;
      console.log('✅ Match: 4 orgs → ' + orgPricingPercent + '%');
    } else if (numOrgs === 5) {
      orgPricingPercent = freshMultiOrgPricing.fiveOrgs;
      console.log('✅ Match: 5 orgs → ' + orgPricingPercent + '%');
    } else if (numOrgs >= 6) {
      orgPricingPercent = freshMultiOrgPricing.sixPlusOrgs;
      console.log('✅ Match: 6+ orgs → ' + orgPricingPercent + '%');
    }

    console.log(`✅ For ${numOrgs} org(s), using ${orgPricingPercent}% pricing`);
  ...
  useEffect(() => {
    if (pricingCalculation.finalAmount >= 0) {
      console.log('💵 Setting final amount:', pricingCalculation.finalAmount.toFixed(2));
  ...
  useEffect(() => {
    if (open && selectedData?.subscription) {
      ...
      console.log('📝 Resetting form with data:', mappedData);
      reset(mappedData);
  ...
  const handleSubmit = async (formData: SubscriptionFormValues) => {
    try {
      ...
      if (isPricingAffected && !dirtyFields.totalSubscriptionAmount) {
        updatedSubscriptionFields.totalSubscriptionAmount = Number(formData.totalSubscriptionAmount);
        console.log('💰 Auto-including totalSubscriptionAmount due to pricing-affecting field changes');
      }

      console.log('📤 Sending only updated fields:', updatedSubscriptionFields);
...
// //   const selectedSubscriptionTypes = useMemo(() => watchedSubscriptionTypes || [], [watchedSubscriptionTypes]);
// //   // CRITICAL FIX: Convert to number explicitly
// //   const numberOfOrganizations = useMemo(() => {
// //     const num = Number(watchedNumberOfOrganizations) || 1;
// //     console.log('🔢 Number of Organizations watched (converted to number):', num, 'Type:', typeof num);
// //     return num;
...
// //   // Extract pricing configuration from pricingData - NO useMemo, always fresh
// //   const modulePrices = (() => {
// //     if (!pricingData?.modulePricing) return {};
...
// //     // Step 1: Calculate base module price
// //     const baseModulePrice = selectedSubscriptionTypes.reduce((total, type) => {
// //       return total + (modulePrices[type] || 0);
// //     }, 0);

// //     // Step 2: Determine bundle discount based on number of non-analytics modules
// //     const nonAnalyticsModules = selectedSubscriptionTypes.filter((type) => type !== 'analytics');
// //     const nonAnalyticsCount = nonAnalyticsModules.length;
...
// //     // Step 3: Get organization pricing percentage - DIRECTLY from pricingData each time
// //     let orgPricingPercent = 100;

// //     // Always get fresh values from pricingData
// //     const freshMultiOrgPricing = {
// //       oneOrg: pricingData?.multiOrgPricing?.oneOrg || 100,
// //       twoOrgs: pricingData?.multiOrgPricing?.twoOrgs || 95,
// //       threeOrgs: pricingData?.multiOrgPricing?.threeOrgs || 90,
...
// //     // CRITICAL: Ensure numberOfOrganizations is a NUMBER for comparison
// //     const numOrgs = Number(numberOfOrganizations);
// //     console.log('🎯 Checking conditions with numOrgs:', numOrgs, 'Type:', typeof numOrgs);
...
// //     // Step 4: Calculate multi-org price
// //     const pricePerOrg = (priceAfterBundleDiscount * orgPricingPercent) / 100;
// //     const totalMultiOrgPrice = pricePerOrg * numOrgs;

// //     // Step 5: Apply yearly discount if applicable
// //     let yearlyDiscountAmount = 0;
// //     const finalMonthlyPrice = totalMultiOrgPrice;
// //     let finalYearlyPrice = totalMultiOrgPrice * 12;
