...
import { useDeleteServingMutation, useGetServingsQuery } from '@/store/Reducer/serving-api';
...
const ServingView = () => {
  ...
  const { data, isLoading, isFetching } = useGetServingsQuery({
    page: page - 1,
    limit,
    search,
  ...
  const [deleteServing, { isLoading: deleteLoading }] = useDeleteServingMutation();

  const servings: ServingRecord[] = data?.data || [];
  ...
  const onDelete = async () => {
    try {
      await deleteServing(selectedId).unwrap();
  ...
  return (
    <div>
      ...
      {openModal.value && (
        <ServingModal open={openModal.value} onClose={openModal.onFalse} isEdit={editModal.value} selectedData={selectedRecord} />
      )}
