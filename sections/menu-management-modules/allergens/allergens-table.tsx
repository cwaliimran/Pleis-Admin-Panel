...
import AllergenTableRow from './allergens-table-row';
import { SamplePageProps } from './types';

const HEAD_LABEL = [
  { id: 'code', label: 'Code', align: 'left', sortable: true, sortKey: 'code' },
  { id: 'name', label: 'Allergen', align: 'left', sortable: true, sortKey: 'name' },
  { id: 'createdAt', label: 'Created At', align: 'left' },
  { id: 'status', label: 'Status', align: 'left' },
  { id: 'actions', label: 'Action', align: 'center' },
];

const AllergenTable: FC<SamplePageProps> = ({
  data = [],
  meta,
  totalCount,
  loading,
      ...
      <div className="grid grid-cols-12">
        <Card className="dark:bg-secondary col-span-12 mt-5 mb-5 px-2 shadow-md md:px-8 lg:col-span-12">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="ml-2 flex items-baseline gap-2 md:ml-0">
              <h3 className="text-xl font-semibold">Allergen List</h3>
              <span className="text-muted-foreground text-sm">EU regulated — {totalCount} mandatory allergens</span>
            </div>
          ...
          <div className="mt-3 min-h-[45vh] rounded-lg border">
            <Table className="w-full rounded-md border">
              ...
              <TableBodyWrapper loading={loading} colSpan={HEAD_LABEL.length} dataLength={data?.length || 0}>
                {data?.map((item, idx) => (
                  <AllergenTableRow key={item?._id || idx} item={item} handleDelete={handleDelete} handleEdit={handleEdit} />
                ))}
              </TableBodyWrapper>
            </Table>
          </div>
