...
import SubcategoryTypeTableRow from './subcategory-types-table-row';
import { SamplePageProps } from './types';

const HEAD_LABEL = [
  { id: 'name', label: 'Name', align: 'left', sortable: true, sortKey: 'name' },
  { id: 'subCategory', label: 'Subcategory', align: 'left', sortable: true, sortKey: 'subCategory.name' },
  { id: 'category', label: 'Category', align: 'left', sortable: true, sortKey: 'category.title' },
  { id: 'createdAt', label: 'Created At', align: 'left', sortable: true, sortKey: 'createdAt' },
  { id: 'status', label: 'Status', align: 'left' },
  { id: 'actions', label: 'Action', align: 'center' },
];

const SubcategoryTypeTable: FC<SamplePageProps> = ({
  data = [],
  meta,
  loading,
  handleDelete,
      ...
      <div className="grid grid-cols-12">
        <Card className="dark:bg-secondary col-span-12 mt-5 mb-5 px-2 shadow-md md:px-8 lg:col-span-12">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <h3 className="ml-2 text-xl font-semibold md:ml-0">Type List</h3>
                ...
                <FormProvider {...methods}>
                  <form className="flex flex-col gap-6 px-4 py-2">
                    <TableFilters
                      ...
                      dateFilter={{
                        id: 'subcategory-type-created-date',
                        label: 'Created date',
          ...
          <div className="mt-3 min-h-[45vh] rounded-lg border">
            <Table className="w-full rounded-md border">
              ...
              <TableBodyWrapper loading={loading} colSpan={HEAD_LABEL.length} dataLength={data?.length || 0}>
                {data?.map((item, idx) => (
                  <SubcategoryTypeTableRow key={item?._id || idx} item={item} handleDelete={handleDelete} handleEdit={handleEdit} />
                ))}
              </TableBodyWrapper>
            </Table>
          </div>
