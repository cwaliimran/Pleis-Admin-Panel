...
const PresetTypeModal = ({ open, onClose, isEdit = false, selectedData }: PresetTypeModalProps) => {
  ...
  const handleSubmit = async (formData: PresetTypeFormValues) => {
    try {
      ...
      // Optional text fields: on edit a cleared field must be sent as '' so the backend
      // overwrites it — omitting the key (what `undefined` becomes once the body is
      // serialised) reads as "unchanged" and leaves the old value on the record. On create
      // there is nothing to clear, so an empty field stays omitted.
      const optionalText = (value?: string) => (isEdit ? (value ?? '') : value || undefined);
      ...
      const payload: any = {
        ...
        example: optionalText(formData.example),
