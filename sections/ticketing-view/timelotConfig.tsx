...
const TimeSlotConfigModal: React.FC<TimeSlotConfigModalProps> = ({ open, onClose, onSave, totalQuantity = 0 }) => {
  ...
  const [timeSlots, setTimeSlots] = React.useState<TimeSlot[]>([{ id: '1', startTime: '09:00', endTime: '11:00', quantity: 0 }]);
  ...
  const updateTimeSlot = (id: string, field: 'startTime' | 'endTime' | 'quantity', value: string | number) => {
    setTimeSlots(timeSlots.map((slot) => (slot.id === id ? { ...slot, [field]: value } : slot)));
  };

  const getTotalAllocated = () => {
    return timeSlots.reduce((sum, slot) => sum + (slot.quantity || 0), 0);
  };
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="mb-3 flex items-center justify-between">
                <label className="text-sm font-medium">Time Slots & Per-Slot Quantity</label>
                <button
                  type="button"
                  onClick={addTimeSlot}
                  ...
                  >
                    ...
                    <div className="relative w-24">
                      ...
                      <input
                        ...
                        onChange={(e) => updateTimeSlot(slot.id, 'quantity', parseInt(e.target.value) || 0)}
                        placeholder="Qty"
                        className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-700 focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200"
                      />
                    </div>
            ...
            <div className="flex justify-end gap-3 border-t pt-4">
              ...
              <Button
                ...
                disabled={totalQuantity > 0 && remainingQuantity < 0}
              >
                Save Configuration
              </Button>
            </div>
