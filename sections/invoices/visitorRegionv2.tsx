    ...
    <div className="w-full h-[270px]">
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Bar
            dataKey="fixed"
            fill={chartConfig.fixed.color}
            radius={[10, 10, 0, 0]}
            name={chartConfig.fixed.label}
          />
          {/* Prepay */}
          <Bar
            dataKey="prepay"
            fill={chartConfig.prepay.color}
            radius={[10, 10, 0, 0]}
            name={chartConfig.prepay.label}
          />
          {/* Min Spend */}
          {chartConfig.minSpend && (
            <Bar
              dataKey="minSpend"
              fill={chartConfig.minSpend.color}
              radius={[10, 10, 0, 0]}
              name={chartConfig.minSpend.label}
            />
          )}
        </BarChart>
      </ResponsiveContainer>
